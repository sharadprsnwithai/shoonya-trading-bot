...
def run_simulation(mode="BUY", sl_pct=0.20, tp_pct=0.50, adx_threshold=20.0):
    lot_size = 65
    lots = 1
    qty = lots * lot_size
    delta = 0.50
    ...
    # In Option Selling, theta decay works in favor (+1.0 pt per hour held)
    # In Option Buying, theta decay works against (-1.0 pt per hour held)
    theta_per_hour = 1.0 if mode == "SELL" else -1.0
    ...
    for day_idx, day in enumerate(sorted_days):
        ...
        for c in day_candles:
            ...
            if t >= datetime.time(15, 5) and open_pos is not None:
                ...
                pos_type = open_pos['type'] # "BUY_CE", "BUY_PE", "SELL_PE", "SELL_CE"
                
                # Spot movement calculation
                # Bullish: Spot going UP is profitable
                # Bearish: Spot going DOWN is profitable
                is_bullish = ("CE" in pos_type and mode == "BUY") or ("PE" in pos_type and mode == "SELL")
            ...
            if open_pos is not None:
                ...
                # Reversal:
                # If Bullish pos: exit when 5m RSI < 15m RSI
                # If Bearish pos: exit when 5m RSI > 15m RSI
            ...
            if not trade_executed_today and t <= datetime.time(15, 0):
                ...
                if bullish and adx_ok:
                    label = "BUY_CE" if mode == "BUY" else "SELL_PE"
                    open_pos = {'type': label, 'entry_time': c['time'], 'entry_dt': c['dt'], 'entry_spot': c['close']}
                    trade_executed_today = True
                elif bearish and adx_ok:
                    label = "BUY_PE" if mode == "BUY" else "SELL_CE"
                    open_pos = {'type': label, 'entry_time': c['time'], 'entry_dt': c['dt'], 'entry_spot': c['close']}
                    trade_executed_today = True
    ...
    return {
        'mode': mode, 'trades_count': len(trades), 'wins': wins, 'losses': losses, 'win_rate': wr,
        'gross_profit': gp, 'gross_loss': gl, 'net_pnl': net_pnl, 'profit_factor': pf,
        'max_dd': max_dd, 'trades': trades
    }

buy_res = run_simulation("BUY")
sell_res = run_simulation("SELL")

print("\n" + "=" * 110)
print(f"{'#':<3} | {'Date':<10} | {'Buying Action':<13} | {'Selling Action':<14} | {'Spot Entry':<10} | {'Spot Exit':<10} | {'Buying PnL':<12} | {'Selling PnL':<12}")
print("=" * 110)

for i in range(len(buy_res['trades'])):
    bt = buy_res['trades'][i]
    st = sell_res['trades'][i]
    print(f"{bt['id']:<3} | {bt['date']:<10} | {bt['type']:<13} | {st['type']:<14} | Rs {bt['entry_spot']:<9.2f} | Rs {bt['exit_spot']:<9.2f} | Rs {bt['pnl_amount']:+9.2f} | Rs {st['pnl_amount']:+9.2f}")

print("=" * 110)

print("\n=== SIDE-BY-SIDE STRATEGY COMPARISON: OPTION BUYING VS OPTION SELLING ===")
print("-" * 75)
print(f"{'Performance Metric':<32} | {'Option Buying (Buy CE/PE)':<20} | {'Option Selling (Sell PE/CE)'}")
print("-" * 75)
print(f"{'Total Trades Evaluated':<32} | {buy_res['trades_count']:<20} | {sell_res['trades_count']}")
print(f"{'Winning Trades':<32} | {buy_res['wins']} ({buy_res['win_rate']:.1f}%)" + " " * 10 + f" | {sell_res['wins']} ({sell_res['win_rate']:.1f}%)")
print(f"{'Losing Trades':<32} | {buy_res['losses']} ({100-buy_res['win_rate']:.1f}%)" + " " * 9 + f" | {sell_res['losses']} ({100-sell_res['win_rate']:.1f}%)")
print(f"{'Gross Profit':<32} | Rs. {buy_res['gross_profit']:<16,.2f} | Rs. {sell_res['gross_profit']:,.2f}")
print(f"{'Gross Loss':<32} | Rs. {buy_res['gross_loss']:<16,.2f} | Rs. {sell_res['gross_loss']:,.2f}")
print(f"{'Net Realized P&L':<32} | Rs. {buy_res['net_pnl']:<16.2f} | Rs. {sell_res['net_pnl']:,.2f}")
print(f"{'Profit Factor':<32} | {buy_res['profit_factor']:<20.2f} | {sell_res['profit_factor']:.2f}")
print(f"{'Max Drawdown':<32} | Rs. {buy_res['max_dd']:<16,.2f} | Rs. {sell_res['max_dd']:,.2f}")
print("-" * 75)
