...
def run_simulation(max_trades_per_day=2, sl_pct=0.02, tp_pct=0.50, adx_threshold=20.0):
    lot_size = 65
    lots = 1
    qty = lots * lot_size
    delta = 0.50
    ...
    for day_idx, day in enumerate(sorted_days):
        ...
        for c in day_candles:
            ...
            # 2. Check Entry
            if trades_today_count < max_trades_per_day and t <= datetime.time(15, 0):
                ...
                if bullish and adx_ok:
                    ...
                    open_pos = {'type': "SELL_PE", 'trade_num': trades_today_count, 'entry_time': c['time'], 'entry_dt': c['dt'], 'entry_spot': c['close']}
                elif bearish and adx_ok:
                    trades_today_count += 1
                    open_pos = {'type': "SELL_CE", 'trade_num': trades_today_count, 'entry_time': c['time'], 'entry_dt': c['dt'], 'entry_spot': c['close']}
    ...
    return {
        'max_trades_per_day': max_trades_per_day, 'trades_count': len(trades), 'wins': wins, 'losses': losses, 'win_rate': wr,
        'gross_profit': gp, 'gross_loss': gl, 'net_pnl': net_pnl, 'profit_factor': pf,
        'max_dd': max_dd, 'trades': trades
    }

res_1 = run_simulation(max_trades_per_day=1, sl_pct=0.02)
res_2 = run_simulation(max_trades_per_day=2, sl_pct=0.02)
res_3 = run_simulation(max_trades_per_day=3, sl_pct=0.02)

print("\n" + "=" * 90)
print("     MAX TRADES PER DAY COMPARISON (OPTION SELLING, 2% SL, 50% TP, ADX >= 20)")
print("=" * 90)
print(f"{'Performance Metric':<28} | {'Max 1 Trade/Day':<18} | {'Max 2 Trades/Day':<18} | {'Max 3 Trades/Day'}")
print("-" * 90)
print(f"{'Total Trades':<28} | {res_1['trades_count']:<18} | {res_2['trades_count']:<18} | {res_3['trades_count']}")
print(f"{'Winning Trades':<28} | {res_1['wins']} ({res_1['win_rate']:.1f}%)" + " " * 8 + f" | {res_2['wins']} ({res_2['win_rate']:.1f}%)" + " " * 8 + f" | {res_3['wins']} ({res_3['win_rate']:.1f}%)")
print(f"{'Losing Trades':<28} | {res_1['losses']} ({100-res_1['win_rate']:.1f}%)" + " " * 7 + f" | {res_2['losses']} ({100-res_2['win_rate']:.1f}%)" + " " * 7 + f" | {res_3['losses']} ({100-res_3['win_rate']:.1f}%)")
print(f"{'Gross Profit':<28} | Rs. {res_1['gross_profit']:<14,.2f} | Rs. {res_2['gross_profit']:<14,.2f} | Rs. {res_3['gross_profit']:,.2f}")
print(f"{'Gross Loss':<28} | Rs. {res_1['gross_loss']:<14,.2f} | Rs. {res_2['gross_loss']:<14,.2f} | Rs. {res_3['gross_loss']:,.2f}")
print(f"{'Net Realized P&L':<28} | Rs. {res_1['net_pnl']:<14,.2f} | Rs. {res_2['net_pnl']:<14,.2f} | Rs. {res_3['net_pnl']:,.2f}")
print(f"{'Profit Factor':<28} | {res_1['profit_factor']:<18.2f} | {res_2['profit_factor']:<18.2f} | {res_3['profit_factor']:.2f}")
print(f"{'Max Drawdown':<28} | Rs. {res_1['max_dd']:<14,.2f} | Rs. {res_2['max_dd']:<14,.2f} | Rs. {res_3['max_dd']:,.2f}")
print("=" * 90)

print("\n=== TRADE LOG FOR MAX 2 TRADES / DAY ===")
print(f"{'#':<3} | {'Date':<10} | {'T#':<2} | {'Action':<8} | {'Entry':<6} | {'Exit':<6} | {'Spot Entry':<10} | {'Spot Exit':<10} | {'P&L (Rs)':<10} | {'Exit Reason'}")
print("-" * 105)
for t in res_2['trades']:
    print(f"{t['id']:<3} | {t['date']:<10} | {t['trade_num']:<2} | {t['type']:<8} | {t['entry_time']:<6} | {t['exit_time']:<6} | Rs {t['entry_spot']:<7.2f} | Rs {t['exit_spot']:<7.2f} | Rs {t['pnl_amount']:+7.2f} | {t['reason']}")
