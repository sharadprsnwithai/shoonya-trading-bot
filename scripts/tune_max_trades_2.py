...
def run_simulation(max_trades=2, sl_pct=0.02, tp_pct=0.50, adx_thresh=20.0):
    lot_size = 65
    qty = 65
    delta = 0.50
    base_premium = 150.0
        ...
        for c in day_candles:
            ...
            if t >= datetime.time(15, 5) and open_pos is not None:
                ...
                trades.append({
                    ...
                    'pnl_amount': pnl, 'reason': 'EOD_SQUARE_OFF'
                })
                open_pos = None
                continue
            ...
            if open_pos is not None:
                ...
                if is_sl_hit or is_tp_hit or is_reversal:
                    ...
                    trades.append({
                        ...
                        'exit_spot': exit_spot, 'pnl_pts': pts, 'pnl_amount': pnl, 'reason': reason
                    })
                    open_pos = None
                continue
                
            # Check entry
            if trades_today_count < max_trades and t <= datetime.time(15, 0):
                ...
                if adx_thresh > 0:
                    ...
                    if adx_val == adx_val and adx_val < adx_thresh:
                        adx_ok = False
    ...
    return {
        'max_trades': max_trades, 'sl_pct': sl_pct, 'tp_pct': tp_pct, 'adx': adx_thresh,
...
for sl in [0.02, 0.03, 0.05]:
    for adx in [18.0, 20.0, 22.0, 25.0]:
        r = run_simulation(max_trades=2, sl_pct=sl, adx_thresh=adx)
        print(f"SL={sl*100:.0f}%, ADX>={adx:.0f} | Trades: {r['trades_count']:<2} (WR: {r['win_rate']:.1f}%) | GP: Rs {r['gross_profit']:<10,.2f} | GL: Rs {r['gross_loss']:<9,.2f} | Net PnL: Rs {r['net_pnl']:<10,.2f} | PF: {r['profit_factor']:.2f} | MaxDD: Rs {r['max_dd']:,.2f}")
