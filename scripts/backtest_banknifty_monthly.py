...
def run_banknifty_backtest(
    df_merged,
    lot_size=30,                    # NSE BANKNIFTY Standard Lot size (30 units)
    num_lots=10,                    # 10 lots = 300 qty
    sl_pts=50.0,                    # Hard SL points on option (~100 pts BankNifty move)
    tp_pts=100.0,                   # Target Profit points (~200 pts BankNifty move)
    trail_trigger1=25.0,            # Step 1 trigger (+25 pts)
    trail_lock1=5.0,                # Step 1 lock (+5 pts breakeven cushion)
    trail_trigger2=55.0,            # Step 2 trigger (+55 pts)
    trail_lock2=30.0,               # Step 2 lock (+30 pts)
    max_vwap_dist=80.0,             # Spot distance to VWAP limit
    ...
    theta_decay_per_hour=5.5        # Monthly Bank Nifty theta decay per hour
):
            ...
            if start_t <= t <= end_t:
                ...
                if bullish_cross:
                    ...
                    current_trade = {
                        ...
                        'direction': 'BULLISH',
                        'action': 'SELL_PE_MONTHLY_CREDIT_SPREAD',
                        'entry_spot': spot,
                        'vwap_at_entry': vwap,
                        'adx_at_entry': adx,
                        'max_fav_pts': 0.0,
                    ...
                    current_trade = {
                        ...
                        'entry_time': timestamp,
                        'direction': 'BEARISH',
                        'action': 'SELL_CE_MONTHLY_CREDIT_SPREAD',
                        'entry_spot': spot,
                        'vwap_at_entry': vwap,
                        'adx_at_entry': adx,
                        'max_fav_pts': 0.0,
...
def main():
    ...
    if df_5m.empty:
        print("Error: No data returned for BANK NIFTY.")
        return
    ...
    print(f"Total evaluated 5-min bars for NSE BANK NIFTY: {len(df_merged)}")
    print(f"Index Price Range: {df_merged['Close'].min():,.1f} - {df_merged['Close'].max():,.1f}")

    print("\n" + "="*105)
    print("NSE BANK NIFTY MONTHLY EXPIRY RSI CROSSOVER BACKTEST (10 LOTS = 300 QTY, LOT SIZE = 30)")
    print("="*105 + "\n")

    # 1. Raw Unfiltered Crossover (Fixed SL/TP, No Trailing)
    r1 = run_banknifty_backtest(
        df_merged,
        use_vwap=False,
    ...
    print_result(r1, "1. Unhedged Raw Crossover (SL 50 pts, TP 100 pts, No Filters, No Trailing)")

    # 2. + VWAP & 15m Supertrend Filters
    r2 = run_banknifty_backtest(
        df_merged,
        use_vwap=True,
        use_supertrend=True,
        use_di=False,
    ...
    # 4. + Stepped Trailing SL (+25 -> +5, +55 -> +30)
    r4 = run_banknifty_backtest(
        df_merged,
        use_vwap=True,
        use_supertrend=True,
        use_di=True,
    ...
    print_result(r4, "4. + Stepped Trailing SL (+25 -> +5, +55 -> +30)")

    # 5. Full CHAMPION SETUP: All Filters + Trailing SL + VWAP Gate <= 80 pts (Max 1 Trade/Day)
    r5 = run_banknifty_backtest(
        df_merged,
        use_vwap=True,
        use_supertrend=True,
        use_di=True,
    ...
    print_result(r5, "5. [CHAMPION SETUP] Full Indicator Filters + Stepped Trailing SL + VWAP Gate <= 80 pts (Max 1 Trade/Day)")

    # 6. CHAMPION SETUP (Max 2 Trades/Day)
    r6 = run_banknifty_backtest(
        df_merged,
        use_vwap=True,
        use_supertrend=True,
        use_di=True,
    ...
    print_result(r6, "6. [CHAMPION SETUP] Full Indicator Filters + Stepped Trailing SL + VWAP Gate <= 80 pts (Max 2 Trades/Day)")

    # 7. Retail Single Lot (30 Qty)
    r7 = run_banknifty_backtest(
        df_merged,
        lot_size=30,
        num_lots=1,
        use_vwap=True,
    ...
    print_result(r7, "7. [CHAMPION SETUP] 1 Lot Retail (30 Qty, Max 1 Trade/Day)")

    # Detailed trade audit log of Champion Setup
    df_trades = r5['trades_df']
    if not df_trades.empty:
        print("="*105)
        print("BANK NIFTY MONTHLY CHAMPION SETUP - DETAILED TRADE AUDIT LOG (LAST 2 MONTHS)")
        print("="*105)
        print(f"{'Date':<12} {'Entry':<6} {'Dir':<8} {'BankNifty':<12} {'VWAP':<10} {'ADX':<6} {'Exit':<6} {'Reason':<16} {'Opt Pts':<10} {'PnL (Rs)':<12}")
        print("-"*105)
    ...
    # Monthly breakdown
    df_t = r5['trades_df'].copy()
    if not df_t.empty:
        df_t['Month'] = df_t['entry_time'].apply(lambda x: x.strftime('%B %Y'))
        print("="*80)
        print("MONTHLY CONSISTENCY BREAKDOWN")
        print("="*80)
        for m, g in df_t.groupby('Month', sort=False):
            wins = g[g['pnl'] > 0]
            wr = len(wins) / len(g) * 100
            pnl = g['pnl'].sum()
            print(f"{m:<16}: {len(g):2d} trades | Win Rate: {wr:5.1f}% | Net PnL: Rs {pnl:+10,.2f}")
        print("="*80 + "\n")
