...
def run_simulation(df_merged, sl_pts, tp_pts, trail_trigger1, trail_lock1, trail_trigger2, trail_lock2, max_vwap_dist, adx_min, daily_max_loss_pts=999.0):
    LOT_SIZE = 65
    NUM_LOTS = 10
    ...
    for i in range(1, len(df_merged)):
        ...
        if current_trade is not None:
            ...
            if opt_pnl_pts <= effective_sl:
                exit_reason = "TRAIL_SL" if effective_sl > -sl_pts else "STOP_LOSS"
                current_trade['exit_time'] = timestamp
                current_trade['exit_spot'] = spot
        ...
        if current_trade is None and trades_today < 2:
            if daily_pnl_pts <= -daily_max_loss_pts:
                continue

            if start_t <= t <= end_t:
                if adx >= adx_min:
                    vwap_dist = spot - vwap
                    if 0 < vwap_dist <= max_vwap_dist and st_dir == 1 and p_di > m_di:
                    ...
                    if 0 < vwap_dist_bear <= max_vwap_dist and st_dir == -1 and m_di > p_di:
...
def main():
    ...
    print("=========================================================================================================")
    print("GRID SEARCH: MINIMIZING DRAWDOWN & MAXIMIZING PROFIT FACTOR")
    print("=========================================================================================================")

    results = []
    for sl in [25.0, 28.0, 30.0, 35.0]:
        for adx in [20.0, 22.0, 25.0]:
            for max_dist in [30.0, 35.0, 45.0, 999.0]:
                for (t1, l1, t2, l2) in [
                    (15.0, 2.0, 28.0, 18.0),
                    (12.0, 2.0, 25.0, 15.0),
                    (18.0, 3.0, 30.0, 20.0),
                    (0.0, 0.0, 0.0, 0.0) # No trailing
                ]:
                    res = run_simulation(
                        df_merged,
                        sl_pts=sl,
    ...
    # Filter for low drawdown and high profit factor
    ...
    print("\n--- TOP 10 LOWEST DRAWDOWN CONFIGURATIONS ---")
    for _, r in top_dd.iterrows():
        print(f"SL:{r['sl']:<2} ADX:{r['adx']:<2} Dist:{r['dist']:<4} Trail:{r['trail']:<18} | Trades:{r['trades']:<2} Win:{r['win_rate']:<4.1f}% | PnL:+Rs {r['pnl']:<10,.2f} | PF:{r['pf']:<4.2f} | DD:-Rs {abs(r['max_dd']):<8,.2f}")

    print("\n--- TOP 10 HIGHEST PnL & RISK-ADJUSTED CONFIGURATIONS ---")
