...
def run_simulation(mode="SELL", sl_pct=0.05, tp_pct=0.50, adx_threshold=20.0):
    lot_size = 65
    lots = 1
    qty = lots * lot_size
    delta = 0.50
        ...
        for c in day_candles:
            ...
            if open_pos is not None:
                ...
                if is_sl_hit or is_tp_hit or is_reversal:
                    if is_sl_hit:
                        reason = f'HARD_SL_HIT_5pct ({pts:.1f} pts)'
                    elif is_tp_hit:
    ...
    return {
        'mode': mode, 'sl_pct': sl_pct, 'trades_count': len(trades), 'wins': wins, 'losses': losses, 'win_rate': wr,
        'gross_profit': gp, 'gross_loss': gl, 'net_pnl': net_pnl, 'profit_factor': pf,
        'max_dd': max_dd, 'trades': trades
    }

res_5pct = run_simulation(mode="SELL", sl_pct=0.05)
res_20pct = run_simulation(mode="SELL", sl_pct=0.20)

print("\n=== OPTION SELLING: 5% STOP-LOSS VS 20% STOP-LOSS COMPARISON ===")
print("-" * 80)
print(f"{'Performance Metric':<32} | {'Option Selling (5% SL)':<20} | {'Option Selling (20% SL)'}")
print("-" * 80)
print(f"{'Total Trades Evaluated':<32} | {res_5pct['trades_count']:<20} | {res_20pct['trades_count']}")
print(f"{'Winning Trades':<32} | {res_5pct['wins']} ({res_5pct['win_rate']:.1f}%)" + " " * 10 + f" | {res_20pct['wins']} ({res_20pct['win_rate']:.1f}%)")
print(f"{'Losing Trades':<32} | {res_5pct['losses']} ({100-res_5pct['win_rate']:.1f}%)" + " " * 9 + f" | {res_20pct['losses']} ({100-res_20pct['win_rate']:.1f}%)")
print(f"{'Gross Profit':<32} | Rs. {res_5pct['gross_profit']:<16,.2f} | Rs. {res_20pct['gross_profit']:,.2f}")
print(f"{'Gross Loss':<32} | Rs. {res_5pct['gross_loss']:<16,.2f} | Rs. {res_20pct['gross_loss']:,.2f}")
print(f"{'Net Realized P&L':<32} | Rs. {res_5pct['net_pnl']:<16,.2f} | Rs. {res_20pct['net_pnl']:,.2f}")
print(f"{'Profit Factor':<32} | {res_5pct['profit_factor']:<20.2f} | {res_20pct['profit_factor']:.2f}")
print(f"{'Max Peak-to-Trough Drawdown':<32} | Rs. {res_5pct['max_dd']:<16,.2f} | Rs. {res_20pct['max_dd']:,.2f}")
print("-" * 80)

print("\n=== TRADE LOG (5% STOP-LOSS) ===")
print(f"{'#':<3} | {'Date':<10} | {'Action':<10} | {'Spot Entry':<10} | {'Spot Exit':<10} | {'P&L (Rs)':<10} | {'Exit Reason'}")
print("-" * 95)
for t in res_5pct['trades']:
