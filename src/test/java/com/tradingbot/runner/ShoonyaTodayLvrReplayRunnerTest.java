...
class ShoonyaTodayLvrReplayRunnerTest {
    ...
    private static final Logger log = LoggerFactory.getLogger(ShoonyaTodayLvrReplayRunnerTest.class);
    ...
    void testReplayToday5mCandles() {
        ...
        System.out.println("[REPLAY] Fetching 5m candles for " + allUniverseSymbols.size() + " sector F&O stocks from Shoonya...");
        ...
        for (String sym : allUniverseSymbols) {
            try {
                ...
                if (candles != null && !candles.isEmpty()) {
                    List<Candle> todayList = candles.stream()
        ...
        System.out.println("[REPLAY] Successfully loaded today's 5m candles for " + stockCandlesToday.size() + " stocks.");
        ...
        List<String> nifty50Symbols = Nifty200Registry.getNifty200Symbols().stream().limit(50).toList();
        ...
        System.out.printf("09:25 IST NIFTY 50 Breadth: %d Advances | %d Declines -> Sentiment: %s\n\n",
                adv, dec, sentiment);

        Map<String, List<StockQuoteSnapshot>> sectorQuotes0925 = new HashMap<>();
        for (Map.Entry<String, List<String>> entry : NiftySectorRegistry.getSectorConstituents().entrySet()) {
        ...
        LowestVolumeReversalScanner.SectorRankResult winningSector = rankedSectors.isEmpty() ? null : rankedSectors.get(0);
        List<String> winningCandidates = new ArrayList<>();
        if (winningSector != null) {
            System.out.printf("\n🏆 WINNING SECTOR: %s (%+.2f%%)\n", winningSector.sectorName(), winningSector.pctChange());
            List<StockQuoteSnapshot> topSectorStockQuotes = sectorQuotes0925.getOrDefault(winningSector.sectorName(), List.of());
        ...
        System.out.println("\n--------------------------------------------------------------------------");
        System.out.println(" STEP 3: UNIVERSE-WIDE SIGNAL AUDIT (LONG & SHORT SETUPS ACROSS ALL 107 STOCKS)");
        System.out.println("--------------------------------------------------------------------------");
        ...
        System.out.println("\n==========================================================================");
        System.out.printf("  SUMMARY: %d LONG Trades Triggered | %d SHORT Trades Triggered\n", longTriggers, shortTriggers);
        System.out.println("==========================================================================");
    }

    private void printCandleDetails(String symbol, List<Candle> candles, LowestVolumeDirection dir) {
        if (candles == null || candles.isEmpty()) {
        ...
        long baseline = Math.min(candles.get(0).volume(), Math.min(candles.get(1).volume(), candles.get(2).volume()));
        System.out.printf("  * Baseline lowest volume (C1..C3): %,d (C1=%,d, C2=%,d, C3=%,d)\n",
                baseline, candles.get(0).volume(), candles.get(1).volume(), candles.get(2).volume());
        ...
        for (int i = 0; i < Math.min(candles.size(), 20); i++) {
            ...
            String tag = (i < 3) ? "[BASELINE IGNORED]" : (isOpposite && lowVol ? ">>> ARMED TRIGGER <<<" : (lowVol ? "[NEW LOW VOL]" : ""));
            System.out.printf("  C%-2d [%s IST] %s | O=%7.2f H=%7.2f L=%7.2f C=%7.2f | Vol=%9d | %s\n",
                    i + 1, time, color, c.open().doubleValue(), c.high().doubleValue(), c.low().doubleValue(), c.close().doubleValue(), c.volume(), tag);
    ...
    private int auditUniverse(Map<String, List<Candle>> stockCandlesToday, LowestVolumeDirection direction) {
        int triggersFound = 0;
        ...
        for (Map.Entry<String, List<Candle>> entry : stockCandlesToday.entrySet()) {
            ...
            long baselineLowest = Math.min(candles.get(0).volume(), Math.min(candles.get(1).volume(), candles.get(2).volume()));
            ...
            for (int i = 3; i < candles.size(); i++) {
                ...
                boolean isOpposite = (direction == LowestVolumeDirection.SHORT) ? c.isGreen() : c.isRed();
                ...
                if (inTrade) {
                    ...
                    if (slHit) {
                        System.out.printf("        [%s IST] 🛑 STOP LOSS HIT for %s at %.2f (SL was %.2f)\n",
                                TIME_FMT.format(c.timestamp()), sym, slPrice.doubleValue(), slPrice.doubleValue());
                        inTrade = false;
                        activeSetup = null;
                    } else if (targetHit && !partialBooked) {
                        partialBooked = true;
                        slPrice = entryPrice; // Move SL to Cost
                        System.out.printf("        [%s IST] 🎯 1:4 TARGET REACHED (50%% Booked) for %s at %.2f | SL moved to Cost (%.2f)\n",
                                TIME_FMT.format(c.timestamp()), sym, targetPrice.doubleValue(), slPrice.doubleValue());
                    }

                    if (candleTime.isAfter(LocalTime.of(15, 10)) && inTrade) {
                        System.out.printf("        [%s IST] 🏁 15:15 HARD EOD EXIT for %s at Close %.2f (Entry was %.2f)\n",
                                TIME_FMT.format(c.timestamp()), sym, c.close().doubleValue(), entryPrice.doubleValue());
                        inTrade = false;
                        break;
                    }
                    continue;
                ...
                if (activeSetup != null && activeSetup.getState() == LowestVolumeSetupState.TRIGGER_ARMED) {
                    boolean breached = false;
                    if (direction == LowestVolumeDirection.SHORT && c.low().compareTo(activeSetup.getTriggerPrice()) <= 0) {
                        breached = true;
                    } else if (direction == LowestVolumeDirection.LONG && c.high().compareTo(activeSetup.getTriggerPrice()) >= 0) {
                        breached = true;
                    }
                    ...
                    if (breached) {
                        ...
                        BigDecimal atmStrike = entryPrice.divide(strikeStep, 0, RoundingMode.HALF_UP).multiply(strikeStep);

                        System.out.printf("\n    🔥 [%s IST] TRADE TRIGGERED: %s %s | BUY %s ATM %.0f%s (Spot Entry: %.2f, SL: %.2f, 1:4 Target: %.2f)\n",
                                TIME_FMT.format(c.timestamp()), sym, direction, sym, atmStrike.doubleValue(), optType,
                                entryPrice.doubleValue(), slPrice.doubleValue(), targetPrice.doubleValue());
                        activeSetup = null;
                        continue;
                    }
                }
                ...
                if (candleTime.isBefore(LocalTime.of(13, 0))) {
                    if (isOpposite && c.volume() < rollingLowest) {
                        ...
                        System.out.printf("      * [%s IST] Setup Armed on %s: %s candle Vol=%,d (< lowest %,d) | Trigger=%.2f, SL=%.2f, 1:4 Target=%.2f\n",
                                TIME_FMT.format(c.timestamp()), sym, c.isGreen() ? "GREEN" : "RED", c.volume(), rollingLowest,
                                trg.doubleValue(), sl.doubleValue(), tgt.doubleValue());
        ...
        System.out.printf("  -> Total Armed Setups: %d | Total Executed Triggers: %d\n", armedSetupsFound, triggersFound);
