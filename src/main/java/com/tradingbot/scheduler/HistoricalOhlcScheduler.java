...
/**
 * Scheduled job to sync historical Daily, Weekly, and Monthly OHLC data from Yahoo Finance
 * at 08:00 AM IST on trading days (Monday - Friday).
 */
@Component
...
public class HistoricalOhlcScheduler {
    ...
    /** Runs at 08:00 AM IST Monday through Friday to ensure the local JSON cache is fully fresh before market open. */
    @Scheduled(cron = "${trading-bot.ohlc.morning-sync-cron:0 0 8 ? * MON-FRI}", zone = "Asia/Kolkata")
    public void runMorningOhlcSync() {
        ...
        try {
            ...
            log.info("[OHLC-SCHEDULER] Morning OHLC sync finished. Total cached symbols: {}", count);
        } catch (Exception e) {
